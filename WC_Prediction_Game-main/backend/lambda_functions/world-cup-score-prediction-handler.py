...
This Lambda function handles three actions:
...
3. FETCH FINISHED predictions (history) - when action='fetch_finished_predictions'
"""
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if action == 'fetch_user_predictions':
            # FETCH action: retrieve ALL predictions for a user
        ...
        elif action == 'fetch_finished_predictions':
            # NEW: FETCH action: retrieve FINISHED predictions for a user (history)
            user_id = body.get('user_id')
            
            if user_id is None:
                print("Missing user_id in fetch_finished_predictions request")
                return {
                    'statusCode': 400,
                    'body': json.dumps({'error': 'Missing user_id'}),
                    'headers': {
            ...
            return fetch_finished_predictions(user_id)
...
def fetch_finished_predictions(user_id):
    """
    Fetch all finished matches with user's predictions (prediction history).
    FIXED: Now checks for status = 'in_progress' OR 'finished'
    
    Args:
        user_id (int): The user ID to fetch predictions for
    
    Returns:
        dict: Response with statusCode and body (list of finished predictions or error)
    """
    connection = None
    try:
        # Validate input
        ...
        # SQL query to fetch finished matches with user predictions
        # FIXED: Now checks for status IN ('finished', 'in_progress')
        sql = """
            SELECT 
                p.prediction_id,
                p.user_id,
            ...
            FROM predictions p
            LEFT JOIN matches m ON p.match_id = m.match_id
            WHERE p.user_id = %s AND (m.status = 'finished' OR m.status = 'in_progress')
            ORDER BY m.match_date_utc DESC
        """
        ...
        print(f"Fetched {len(predictions)} finished predictions for user {user_id}")
        ...
        print(f"Database error while fetching finished predictions: {str(e)}")
        return {
            'statusCode': 500,
            'body': json.dumps({'error': 'Database error'}),
            'headers': {
        ...
        print(f"Unexpected error in fetch_finished_predictions: {str(e)}")
        return {
            'statusCode': 500,
            'body': json.dumps({'error': 'Internal server error'}),
            'headers': {
